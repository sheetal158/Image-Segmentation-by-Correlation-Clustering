...
superpixelfiles = dir('order4/8068.mat'); 
...
for index=1:nfiles
    ...
    spStructure = load(strcat('order4/',currentfilename)); % get the struct from saved superpixel mat file
    spMatrix = spStructure.superpixels_o4; % get the super pixel matrix
 ...
 graph = load(strcat('order4_graph/',str1));
 [x y z] = find(graph.Am);
 FeatureVector_temp=[];
 for i=1:size(x,1)
 ...
 matFileName = strcat('order4_features/',currentfilename);
 save(matFileName, 'FeatureVector_temp');
end
%saving the feature vector

save('order4_features/Features.mat','FeatureVector');
end
