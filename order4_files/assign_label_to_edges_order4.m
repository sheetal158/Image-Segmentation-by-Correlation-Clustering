function assign_label_to_edges_order4()
...
for index=1:nfiles
    ...
    FeaturesMat = load(strcat('order4_features/',currentfilename));
    ...
    pathTosuperpixelGraph = strcat('order4_graph/',str2);
    superpixelGraphMat = load(pathTosuperpixelGraph);
    [x y z] = find(superpixelGraphMat.Am);

    fid = fopen(strcat('extracted_edge_labels_order4/edge_labels_extracted_o4_',strcat(token,'.txt')),'a');    
    ...
    matFileName = strcat('order4_edge_labels/',str1);
