function assign_label_to_edges_order6()
...
for index=1:nfiles
    ...
    FeaturesMat = load(strcat('order6_features/',currentfilename));
    ...
    pathTosuperpixelGraph = strcat('order6_graph/',str2);
    superpixelGraphMat = load(pathTosuperpixelGraph);
    [x y z] = find(superpixelGraphMat.Am);

    fid = fopen(strcat('extracted_edge_labels_order6/edge_labels_extracted_o6_',strcat(token,'.txt')),'a');    
    ...
    matFileName = strcat('order6_edge_labels/',str1);
