...
for index=1:nfiles
    ...
    spStructure = load(strcat('order2/',currentfilename)); % get the struct from saved superpixel mat file
    spMatrix = spStructure.superpixels_o2; % get the super pixel matrix
 ...
 graph = load(strcat('order2_graph/',str1));
 [x y z] = find(graph.Am);
 FeatureVector_temp=[];
 for i=1:size(x,1)
 ...
 matFileName = strcat('order2_features/',currentfilename);
 save(matFileName, 'FeatureVector_temp');
end
%saving the feature vector

save('order2_features/Features.mat','FeatureVector');
end
