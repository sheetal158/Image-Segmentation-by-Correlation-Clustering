function superpixels_order7()

r = randi([1 255],1,1000);
g = randi([1 255],1,1000);
b = randi([1 255],1,1000);

segmentFileMat = load(strcat('order7/8068.mat'));
color_index=1;
superpixels = segmentFileMat.superpixels_o7;

superpixels_o8 = zeros(size(superpixels,1),size(superpixels,2));
segmentedImage = zeros(size(superpixels,1),size(superpixels,2),3);

fid = fopen('order7_clusters/clusters_o7_8068.txt');
...
save(matFileName, 'superpixels_o8');
