...
for index=1:nfiles
    ...
    spStructure = load(strcat('order7/',currentfilename)); % get the struct from saved superpixel mat file
    spMatrix = spStructure.superpixels_o7; % get the super pixel matrix
 ...
 graph = load(strcat('order7_graph/',str1));
 [x y z] = find(graph.Am);
 FeatureVector_temp=[];
 for i=1:size(x,1)
 ...
 matFileName = strcat('order7_features/',currentfilename);
 save(matFileName, 'FeatureVector_temp');
end
%saving the feature vector

save('order7_features/Features.mat','FeatureVector');
end
